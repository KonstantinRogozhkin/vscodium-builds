...
var CHAR_UPPERCASE_A, CHAR_LOWERCASE_A, CHAR_UPPERCASE_Z, CHAR_LOWERCASE_Z, CHAR_DOT, CHAR_FORWARD_SLASH, CHAR_BACKWARD_SLASH, CHAR_COLON, CHAR_QUESTION_MARK, ErrorInvalidArgType, platformIsWin32, win32, posixCwd, posix, normalize, isAbsolute, join2, resolve, relative2, dirname3, basename, extname, format, parse, toNamespacedPath, sep, delimiter;
var init_path = __esm({
  "out-build/vs/base/common/path.js"() {
    "use strict";
    init_process();
    ...
    win32 = {
      ...
      normalize(path3) {
        ...
        let tail = rootEnd < len ? normalizeString(path3.slice(rootEnd), !isAbsolute2, "\\", isPathSeparator) : "";
        ...
        if (!isAbsolute2 && device === void 0 && path3.includes(":")) {
          if (tail.length >= 2 && isWindowsDeviceRoot(tail.charCodeAt(0)) && tail.charCodeAt(1) === CHAR_COLON) {
            return `.\\${tail}`;
          }
          let index = path3.indexOf(":");
          do {
            if (index === len - 1 || isPathSeparator(path3.charCodeAt(index + 1))) {
              return `.\\${tail}`;
            }
          } while ((index = path3.indexOf(":", index + 1)) !== -1);
        }
        if (device === void 0) {
    ...
    posix = {
      ...
      normalize(path3) {
        ...
        const isAbsolute2 = path3.charCodeAt(0) === CHAR_FORWARD_SLASH;
        const trailingSeparator = path3.charCodeAt(path3.length - 1) === CHAR_FORWARD_SLASH;
        path3 = normalizeString(path3, !isAbsolute2, "/", isPosixPathSeparator);
        if (path3.length === 0) {
          if (isAbsolute2) {
            return "/";
          }
      ...
      join(...paths) {
        ...
        return posix.normalize(path3.join("/"));
      },
      relative(from, to) {
        validateString(from, "from");
        validateString(to, "to");
...
function hasDriveLetter(path3, isWindowsOS = isWindows2) {
  if (isWindowsOS) {
...
async function rimrafMove(path3, moveToPath = randomPath(tmpdir())) {
  try {
    try {
      await fs3.promises.rename(path3, moveToPath);
    } catch (error) {
...
async function rimrafUnlink(path3) {
  return fs3.promises.rm(path3, { recursive: true, force: true, maxRetries: 3 });
}
async function readdir(path3, options) {
  return handleDirectoryChildren(await (options ? safeReaddirWithFileTypes(path3) : fs3.promises.readdir(path3)));
}
async function safeReaddirWithFileTypes(path3) {
  try {
    return await fs3.promises.readdir(path3, { withFileTypes: true });
  } catch (error) {
  ...
  for (const child of children) {
    ...
    try {
      const lstat = await fs3.promises.lstat(join2(path3, child));
      isFile = lstat.isFile();
...
function doWriteFileAndFlush(path3, data, options, callback) {
  if (!canFlush) {
    return fs3.writeFile(path3, data, { mode: options.mode, flag: options.flag }, callback);
  }
  fs3.open(path3, options.flag, options.mode, (openError, fd) => {
    if (openError) {
...
function writeFileSync2(path3, data, options) {
  const ensuredOptions = ensureWriteOptions(options);
  if (!canFlush) {
    return fs3.writeFileSync(path3, data, { mode: ensuredOptions.mode, flag: ensuredOptions.flag });
  }
  const fd = fs3.openSync(path3, ensuredOptions.flag, ensuredOptions.mode);
  try {
...
async function realpath2(path3) {
  try {
    return await promisify(fs3.realpath)(path3);
  } catch (error) {
    const normalizedPath = normalizePath2(path3);
    await fs3.promises.access(normalizedPath, fs3.constants.R_OK);
    return normalizedPath;
  }
}
function normalizePath2(path3) {
  return rtrim(normalize(path3), sep);
}
  ...
  "out-build/vs/base/node/pfs.js"() {
    ...
    (function(SymlinkSupport2) {
      async function stat(path3) {
        let lstats;
        try {
          lstats = await fs3.promises.lstat(path3);
        ...
        try {
          const stats = await fs3.promises.stat(path3);
          ...
          if (isWindows2 && error.code === "EACCES") {
            try {
              const stats = await fs3.promises.stat(await fs3.promises.readlink(path3));
...
function wrapRelativePattern(parsedPattern, arg2) {
  ...
  const wrappedPattern = function(path3, basename3) {
    if (!isEqualOrParent(path3, arg2.base, !isLinux)) {
      return null;
    }
    return parsedPattern(ltrim(path3.substr(arg2.base.length), sep), basename3);
  };
...
function trivia1(base, pattern) {
  return function(path3, basename3) {
...
function trivia2(base, pattern) {
  ...
  const parsedPattern = function(path3, basename3) {
...
function trivia3(pattern, options) {
  ...
  const parsedPattern = function(path3, basename3) {
    for (let i = 0, n = parsedPatterns.length; i < n; i++) {
      if (parsedPatterns[i](path3, basename3)) {
        return pattern;
      }
    }
    return null;
...
function trivia4and5(targetPath, pattern, matchPathEnds) {
  ...
  let parsedPattern;
  if (matchPathEnds) {
    parsedPattern = function(path3, basename3) {
...
function parse2(arg1, options = {}) {
  ...
  if (typeof arg1 === "string" || isRelativePattern(arg1)) {
    ...
    const resultPattern = function(path3, basename3) {
      return !!parsedPattern(path3, basename3);
    };
...
function parsedExpression(expression, options) {
  ...
  if (!parsedPatterns.some((parsedPattern) => !!parsedPattern.requiresSiblings)) {
    ...
    const resultExpression2 = function(path3, basename3) {
      let resultPromises = void 0;
      for (let i = 0, n = parsedPatterns.length; i < n; i++) {
        const result = parsedPatterns[i](path3, basename3);
  ...
  const resultExpression = function(path3, base, hasSibling) {
    ...
    for (let i = 0, n = parsedPatterns.length; i < n; i++) {
      ...
      if (parsedPattern.requiresSiblings && hasSibling) {
        ...
        if (!name) {
          name = base.substr(0, base.length - extname(path3).length);
        }
      }
...
function parseExpressionPattern(pattern, value, options) {
  ...
  if (value) {
    ...
    if (typeof when === "string") {
      const result = (path3, basename3, name, hasSibling) => {
        if (!hasSibling || !parsedPattern(path3, basename3)) {
          return null;
        }
...
function aggregateBasenameMatches(parsedPatterns, result) {
  ...
  const aggregate = function(path3, basename3) {
    if (typeof path3 !== "string") {
      return null;
    }
    if (!basename3) {
    ...
    ByteSize = class _ByteSize {
      ...
      static formatSize(size) {
        ...
        if (size < _ByteSize.KB) {
          return localize(2042, null, size.toFixed(0));
        }
        if (size < _ByteSize.MB) {
          return localize(2043, null, (size / _ByteSize.KB).toFixed(2));
        }
        if (size < _ByteSize.GB) {
          return localize(2044, null, (size / _ByteSize.MB).toFixed(2));
        }
        if (size < _ByteSize.TB) {
          return localize(2045, null, (size / _ByteSize.GB).toFixed(2));
        }
        return localize(2046, null, (size / _ByteSize.TB).toFixed(2));
      }
    };
  }
});
...
function normalizeWatcherPattern(path3, pattern) {
  if (typeof pattern === "string" && !pattern.startsWith(GLOBSTAR) && !isAbsolute(pattern)) {
    return { base: path3, pattern };
  }
  return pattern;
}
function parseWatcherPatterns(path3, patterns) {
  const parsedPatterns = [];
  for (const pattern of patterns) {
    parsedPatterns.push(parse2(normalizeWatcherPattern(path3, pattern)));
  }
  return parsedPatterns;
}
...
async function watchFileContents(path3, onData, onReady, token, bufferSize = 512) {
  const handle = await Promises2.open(path3, "r");
  const buffer = Buffer.allocUnsafe(bufferSize);
  const cts = new CancellationTokenSource(token);
  let error = void 0;
  let isReading = false;
  const request = { path: path3, excludes: [], recursive: false };
...
var init_nodejsWatcherLib = __esm({
  "out-build/vs/platform/files/node/watcher/nodejs/nodejsWatcherLib.js"() {
    ...
    NodeJSFileWatcherLibrary = class _NodeJSFileWatcherLibrary extends Disposable {
      ...
      async existsChildStrictCase(path3) {
        if (isLinux) {
          return Promises2.exists(path3);
        }
        try {
          const pathBasename = basename(path3);
          const children = await Promises2.readdir(dirname3(path3));
      ...
      for (const deprecatedId of o.deprecates) {
        if (remainingArgs.hasOwnProperty(deprecatedId)) {
          if (!val) {
            ...
            if (val) {
              errorReporter.onDeprecatedOption(deprecatedId, o.deprecationMessage || localize(1901, null, optionId));
            }
          }
...
function buildHelpMessage(productName, executableName, version, options, capabilities) {
  ...
  const inputFiles = capabilities?.noInputFiles ? "" : capabilities?.isChat ? ` [${localize(1902, null)}]` : ` [${localize(1903, null)}...]`;
  const subcommand = capabilities?.isChat ? " chat" : "";
  const help = [`${productName} ${version}`];
  help.push("");
  help.push(`${localize(1904, null)}: ${executableName}${subcommand} [${localize(1905, null)}]${inputFiles}`);
  help.push("");
...
function buildStdinMessage(executableName, isChat) {
  ...
  return localize(1907, null, example);
}
function buildVersionMessage(version, commit) {
  return `${version || localize(1908, null)}
${commit || localize(1909, null)}
${process.arch}`;
}
...
var init_argv = __esm({
  "out-build/vs/platform/environment/node/argv.js"() {
    ...
    init_nls();
    helpCategories = {
      o: localize(1850, null),
      e: localize(1851, null),
      t: localize(1852, null),
      m: localize(1853, null)
    };
    ...
    OPTIONS = {
      "chat": {
        ...
        options: {
          "_": { type: "string[]", description: localize(1854, null) },
          "mode": { type: "string", cat: "o", alias: "m", args: "mode", description: localize(1855, null) },
          "add-file": { type: "string[]", cat: "o", alias: "a", args: "path", description: localize(1856, null) },
          "maximize": { type: "boolean", cat: "o", description: localize(1857, null) },
          "reuse-window": { type: "boolean", cat: "o", alias: "r", description: localize(1858, null) },
          "new-window": { type: "boolean", cat: "o", alias: "n", description: localize(1859, null) },
          "help": { type: "boolean", alias: "h", description: localize(1860, null) }
        }
      },
      "serve-web": {
        type: "subcommand",
        ...
        options: {
          "cli-data-dir": { type: "string", args: "dir", description: localize(1861, null) },
      ...
      "tunnel": {
        ...
        options: {
          "cli-data-dir": { type: "string", args: "dir", description: localize(1862, null) },
          "disable-telemetry": { type: "boolean" },
          "telemetry-level": { type: "string" },
          user: {
            type: "subcommand",
      ...
      "diff": { type: "boolean", cat: "o", alias: "d", args: ["file", "file"], description: localize(1863, null) },
      "merge": { type: "boolean", cat: "o", alias: "m", args: ["path1", "path2", "base", "result"], description: localize(1864, null) },
      "add": { type: "boolean", cat: "o", alias: "a", args: "folder", description: localize(1865, null) },
      "remove": { type: "boolean", cat: "o", args: "folder", description: localize(1866, null) },
      "goto": { type: "boolean", cat: "o", alias: "g", args: "file:line[:character]", description: localize(1867, null) },
      "new-window": { type: "boolean", cat: "o", alias: "n", description: localize(1868, null) },
      "reuse-window": { type: "boolean", cat: "o", alias: "r", description: localize(1869, null) },
      "wait": { type: "boolean", cat: "o", alias: "w", description: localize(1870, null) },
      "waitMarkerFilePath": { type: "string" },
      "locale": { type: "string", cat: "o", args: "locale", description: localize(1871, null) },
      "user-data-dir": { type: "string", cat: "o", args: "dir", description: localize(1872, null) },
      "profile": { type: "string", "cat": "o", args: "profileName", description: localize(1873, null) },
      "help": { type: "boolean", cat: "o", alias: "h", description: localize(1874, null) },
      "extensions-dir": { type: "string", deprecates: ["extensionHomePath"], cat: "e", args: "dir", description: localize(1875, null) },
      "extensions-download-dir": { type: "string" },
      "builtin-extensions-dir": { type: "string" },
      "list-extensions": { type: "boolean", cat: "e", description: localize(1876, null) },
      "show-versions": { type: "boolean", cat: "e", description: localize(1877, null) },
      "category": { type: "string", allowEmptyValue: true, cat: "e", description: localize(1878, null), args: "category" },
      "install-extension": { type: "string[]", cat: "e", args: "ext-id | path", description: localize(1879, null) },
      "pre-release": { type: "boolean", cat: "e", description: localize(1880, null) },
      "uninstall-extension": { type: "string[]", cat: "e", args: "ext-id", description: localize(1881, null) },
      "update-extensions": { type: "boolean", cat: "e", description: localize(1882, null) },
      "enable-proposed-api": { type: "string[]", allowEmptyValue: true, cat: "e", args: "ext-id", description: localize(1883, null) },
      "add-mcp": { type: "string[]", cat: "m", args: "json", description: localize(1884, null) },
      "version": { type: "boolean", cat: "t", alias: "v", description: localize(1885, null) },
      "verbose": { type: "boolean", cat: "t", global: true, description: localize(1886, null) },
      "log": { type: "string[]", cat: "t", args: "level", global: true, description: localize(1887, null) },
      "status": { type: "boolean", alias: "s", cat: "t", description: localize(1888, null) },
      "prof-startup": { type: "boolean", cat: "t", description: localize(1889, null) },
      ...
      "disable-extensions": { type: "boolean", deprecates: ["disableExtensions"], cat: "t", description: localize(1890, null) },
      "disable-extension": { type: "string[]", cat: "t", args: "ext-id", description: localize(1891, null) },
      "sync": { type: "string", cat: "t", description: localize(1892, null), args: ["on | off"] },
      "inspect-extensions": { type: "string", allowEmptyValue: true, deprecates: ["debugPluginHost"], args: "port", cat: "t", description: localize(1893, null) },
      "inspect-brk-extensions": { type: "string", allowEmptyValue: true, deprecates: ["debugBrkPluginHost"], args: "port", cat: "t", description: localize(1894, null) },
      "disable-lcd-text": { type: "boolean", cat: "t", description: localize(1895, null) },
      "disable-gpu": { type: "boolean", cat: "t", description: localize(1896, null) },
      "disable-chromium-sandbox": { type: "boolean", cat: "t", description: localize(1897, null) },
      "sandbox": { type: "boolean" },
      "locate-shell-integration-path": { type: "string", cat: "t", args: ["shell"], description: localize(1898, null) },
      "telemetry": { type: "boolean", cat: "t", description: localize(1899, null) },
      ...
      "startup-experiment-group": { type: "string", cat: "t", args: "control|maximizedChat|splitEmptyEditorChat|splitWelcomeChat", description: localize(1900, null) },
      // chromium flags
...
function parseAndValidate(cmdLineArgs, reportWarnings) {
  const onMultipleValues = (id2, val) => {
    console.warn(localize(1910, null, id2, val));
  };
  const onEmptyValue = (id2) => {
    console.warn(localize(1911, null, id2));
  };
  const onDeprecatedOption = (deprecatedOption, message) => {
    console.warn(localize(1912, null, deprecatedOption, message));
  };
  const getSubcommandReporter = (command) => ({
    onUnknownOption: (id2) => {
      if (!NATIVE_CLI_COMMANDS.includes(command)) {
        console.warn(localize(1913, null, id2, command));
      }
    },
    onMultipleValues,
    onEmptyValue,
  ...
  const errorReporter = {
    onUnknownOption: (id2) => {
      console.warn(localize(1914, null, id2));
    },
    onMultipleValues,
    onEmptyValue,
    onDeprecatedOption,
  ...
  if (args.goto) {
    args._.forEach((arg) => assert2(/^(\w:)?[^:]+(:\d*){0,2}:?$/.test(arg), localize(1915, null)));
  }
  return args;
}
...
var init_product = __esm({
  "out-build/vs/platform/product/common/product.js"() {
    ...
    if (typeof vscodeGlobal2 !== "undefined" && typeof vscodeGlobal2.context !== "undefined") {
      ...
      const { serviceUrl, controlUrl, extensionUrlTemplate, resourceUrlTemplate } = product2.extensionsGallery || {};
      ...
      try {
        ...
        product2 = merge(product2, userProduct);
      } catch (ex) {
      }
      if (env["VSCODE_DEV"]) {
        Object.assign(product2, {
...
async function main(argv) {
    ...
    if (args[subcommand]) {
      ...
      return new Promise((resolve2, reject) => {
        ...
        if (process.env["VSCODE_DEV"]) {
          ...
          const appPath = process.platform === "darwin" ? join2(dirname3(dirname3(process.execPath)), "Resources", "app") : dirname3(process.execPath);
  ...
  if (args.help) {
    ...
    if (isWindows2) {
      for (const path3 of [source, target]) {
        if (typeof path3 === "string" && isUNC(path3)) {
          addUNCHostToAllowlist(URI.file(path3).authority);
        }
      }
    }
...
function touch(path3) {
  ...
  return promises3.utimes(path3, date, date);
}
